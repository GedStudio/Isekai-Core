package org.gedstudio.isekai.lib.impl.command;
...
import org.gedstudio.isekai.lib.open.command.EzRegisteredCommand;
...
public final class EzCommandManagerImpl implements Listener, EzCommandManager {
    ...
    private EzCommandManagerImpl() {
        Bukkit.getPluginManager().addPermission(new Permission("isekai.lib.command.registering", PermissionDefault.TRUE));
    }
    ...
    public EzRegisteredCommand register(String prefix, EzCommand ezCommand) {
        ...
        command.setPermission("isekai.lib.command.registering");
    ...
    private static void setDispatcher(Command vanillaListenerWrapper, Object nmsCommandDispatcher) {
        try {
            Field field = VanillaCommandWrapper.class.getDeclaredField("dispatcher");
    ...
    public void onLoad(ServerLoadEvent serverLoadEvent) {
        ...
        CommandDispatcher<CommandListenerWrapper> mojangDispatcher = commandDispatcher.a();
            ...
            for (String prefix : REGISTERED.keySet()) {
                for (EzCommandImpl ezCommand : REGISTERED.get(prefix)) {
                    ...
                    for (String string : aliases) {
                        ...
                        if (aliaCommand.getPermission().equalsIgnoreCase("isekai.lib.command.registering")) {
                        ...
                        if (aliaCmd.getPermission().equalsIgnoreCase("isekai.lib.command.registering")) {
                    ...
                    if (cmd.getPermission().equalsIgnoreCase("isekai.lib.command.registering")) {
